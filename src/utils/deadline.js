...
export function parseDeadlineMs(deadline) {
...
export function isOrderDeadlinePassed(deadline, nowMs = Date.now()) {
...
/** 送出給後端／試算表的顯示格式 */
export function datetimeLocalToDeadlineString(value) {
    const v = value.trim();
    if (!v)
        return "";
    return v.replace("T", " ");
