...
import { getThemeByMonth } from "../themes";
...
function resolveMonth() {
    ...
    if (fromUrl)
        return { month: fromUrl, manual: fromUrl };
    const stored = readThemeFromStorage();
    if (stored)
        return { month: stored, manual: stored };
    return { month: getCurrentMonth(), manual: null };
}
export function useMonthlyTheme() {
    const [{ month, manual }, setState] = useState(() => resolveMonth());
    const setManualMonth = useCallback((next) => {
        try {
            if (next == null) {
    ...
    useEffect(() => {
        ...
        const t = window.setTimeout(() => {
            setState((prev) => prev.manual === null ? { month: getCurrentMonth(), manual: null } : prev);
        }, ms);
